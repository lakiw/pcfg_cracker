...
def parseCommandLine(ruleName,outputFile,numHoneyWords,wordlists):
	...
	parser.add_argument('--dname3','-dn3', help='The name of a supplementary dictionary',required=False, default='')
        parser.add_argument('--dprob3','-dp3', help='The probability of a supplemenatry dictionary',required=False, default=0.0)
	...
	if args['dname3']!='':
		wordlists.append([args['dname3'],float(args['dprob3'])])
...
def readWordlists(wordlists,dictionaryPT):
	...
	if len(wordlists)>1:
		if readRawWordlists(wordlists[1],rawList2)!=0:
                	...
                	return -1
	if len(wordlists)>2:
		if readRawWordlists(wordlists[2],rawList3)!=0:
			print "Problem reading the 3rd dictionary file"
			return -1
	#Add it to the master wordlist
	#First find out the max length and add extra nodes to the shorter list
	if (len(rawList1)>len(rawList2)) and (len(rawList1)>len(rawList3)):
		maxLen =len(rawList1)
		for x in range(maxLen-len(rawList2)):
			rawList2.append((len(rawList2)+x+1,[]))
		for x in range(maxLen-len(rawList3)):
			rawList3.append((len(rawList3)+x+1,[]))
	elif (len(rawList2)>len(rawList1)) and (len(rawList2)>len(rawList3)):
		maxLen =len(rawList2)
		...
		maxLen =len(rawList3)
                for x in range(maxLen-len(rawList1)):
                        rawList1.append((len(rawList1)+x+1,[]))
                for x in range(maxLen-len(rawList2)):
                        rawList3.append((len(rawList2)+x+1,[]))
...
####################################################
def readGrammar(ruleName,basePT, specialPT, digitPT, capPT, keyboardPT,replacePT,contextPT):
	...
	if (readLenPT(ruleName,"/Keyboard/",keyboardPT)!=0):
                ...
                return -1
	if (readLenPT(ruleName,"/Replace/",replacePT)!=0):
		print "Error reading in the replacement files"
		return -1
	if (readLenPT(ruleName,"/Context/",contextPT)!=0):
		print "Error reading in the context sensitive files"
                return -1
	return 0
...
###################################################################
# Applies a letter replacement
###################################################################
def findReplacement(wordList,replaceList):
	replacement=findRandom(replaceList)
	origValue = replacement[0]
	replaceValue = replacement[2]
	count = 0
	word=[]
	#sanity check to avoid trying to find a word that has a replacevalue if no word meets that criteria
	while count < 1000:
		word = findRandom(wordList)
		if origValue in word:
			break	
		count = count +1
	return word.replace(origValue,replaceValue)
	

###################################################
# Main function to generate honeyords
###################################################
def createHoney(numHoneyWords,basePT,specialPT,digitPT,capPT,keyboardPT,dictionaryPT,replacePT,contextPT,honeyWords):
		...
		while valid_guess==0:
			...
			for y in parsedBase:
				if y[0]=='L':
					...
					#No longer applying masks to 'L' words
					terminal = orig_word
				if y[0]=='D':
				...
				if y[0]=='r':
					#Need to handle the cases where there may not be an appropriately sized dictionary word in the input dictionary
                                        ...
                                        if len(dictionaryPT[y[1]][1][0])<3:
                                                valid_guess=0
                                                break
					orig_word = findReplacement(dictionaryPT[y[1]][1],replacePT[1][1])
                                        #orig_word = findRandom(dictionaryPT[y[1]][1])
                                        if len(orig_word)==0:
                                                valid_guess=0
                                                break
				...
				if y[0]=='R':
					...
					if y[1] >= len(capPT):
						...
						break
                                        ...
                                        terminal = applyMask(orig_word,cap_mask)
				if y[0]=='X':
					terminal = findRandom(contextPT[1][1])
...
def main():
	...
	if readGrammar(ruleName,basePT, specialPT, digitPT, capPT, keyboardPT,replacePT,contextPT)!=0:
		exit()
	if readWordlists(wordlists,dictionaryPT)!=0:
		exit()
	createHoney(numHoneyWords,basePT,specialPT,digitPT,capPT,keyboardPT,dictionaryPT,replacePT,contextPT,honeyWords)
